...
int read_image(Ref<LuminanceSource> source, bool hybrid, string expected, vector<string> &codes) {
...
std::string scan(std::string filename) {
    ...
    cout << "Image reader source created" << endl;
    string expected = read_expected(filename);

    vector<string> codes;
    cout << "Starting to read image" << endl;
    int hresult = read_image(source, true, expected, codes); // hybrid
    if (hresult) {
        read_image(source, false, expected, codes); // global
